...
run_dhcmd "$DHCMD_PATH" "$RP_URI" "SetFeatureFilterDesiredState Gateway.EnableCertificateIssuance.Mqtt 2 $CURRENT_DATE $ALIAS /PaasV2:True" > /dev/null 2>&1
...
run_dhcmd "$DHCMD_PATH" "$RP_URI" "SetFeatureFilter Gateway.EnableCertificateIssuance.Mqtt 1 /PaasV2:True" > /dev/null 2>&1
